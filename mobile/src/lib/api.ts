...
/** The worker profile saved on this phone no longer exists on the server. */
 ...
 */
export async function cachedWorkers(category: string): Promise<NearbyWorker[] | null> {
  const raw = await AsyncStorage.getItem(cacheKey(category)).catch(() => null);
  return raw ? (JSON.parse(raw) as NearbyWorker[]) : null;
}
...
export async function nearbyWorkers(lat: number, lng: number, category: string, lang: Lang) {
  try {
    ...
    const cached = await cachedWorkers(category);
    if (cached) return { workers: cached, stale: true };
    throw e;
  }
}
...
export async function heartbeat(profileId: string, lat: number, lng: number, onDuty: boolean) {
  const { data, error } = await client().rpc('worker_heartbeat', {
    p_profile: profileId,
    p_lat: lat,
    p_lng: lng,
    p_on_duty: onDuty,
  ...
  if (error) throw new BackendError(error.message);
  if (data === false) throw new WorkerGone();
}
