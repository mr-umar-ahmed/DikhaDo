...
import { rateJob, setStatus, useLiveJob, workerUpi, type Job, type Status } from '@/lib/requests';
import { colors, radius, space, touch } from '@/theme/tokens';
import { serialStyle, typeScale } from '@/theme/type';

const steps: Status[] = ['requested', 'accepted', 'on_the_way', 'working', 'done', 'paid', 'rated'];
...
export default function JobScreen() {
  ...
  if (job === null) return <PaperScreen title={t('jobTitle')}><Notice tone="warn" title={t('backendErrorTitle')} /></PaperScreen>;
  ...
  const act = (fn: () => Promise<void>) => fn().then(refresh).catch(() => {});
  ...
  return (
    <PaperScreen title={category?.name[lang] ?? t('jobTitle')} subtitle={workerName}>
      ...
      {ended ? (
        <Notice
          tone="warn"
          title={t(job.status === 'declined' ? 'st_declined' : 'st_cancelled', { name: first })}
          action={t('pickAnother')}
          onAction={() => router.replace({ pathname: '/workers/[code]', params: { code: job.category_code } })}
        />
      ) : (
        <View style={styles.timeline}>
          {steps.slice(0, 5).map((s, i) => {
            const state = i < reached ? 'past' : i === reached ? 'now' : 'next';
            return (
      ...
      {(job.status === 'accepted' || job.status === 'on_the_way' || job.status === 'working') && job.worker?.phone && (
        <PrimaryButton label={`${t('call')} ${first}`} icon="phone" tone="ink" onPress={() => Linking.openURL(`tel:${job.worker!.phone}`).catch(() => {})} />
      )}
      {job.status === 'requested' && (
        <Pressable accessibilityRole="button" onPress={() => act(() => setStatus(job.id, 'cancelled'))} style={styles.link}>
          <Text style={[type.label, { color: colors.registerRed }]}>{t('cancelJob')}</Text>
        </Pressable>
      )}

      {job.status === 'done' && <Pay job={job} onPaid={refresh} />}
      {job.status === 'paid' && <Rate job={job} onRated={refresh} />}
      {job.status === 'rated' && <Notice title={t('st_rated')} action={t('whatIsBroken')} onAction={() => router.replace('/home')} />}
    </PaperScreen>
  );
}

function Pay({ job, onPaid }: { job: Job; onPaid: () => void }) {
  const { lang } = usePrefs();
  const { t } = useTranslation();
  const type = typeScale(lang);
  ...
  const paid = (method: 'cash' | 'upi') => setStatus(job.id, 'paid', { pay_method: method }).then(onPaid).catch(() => {});
  ...
  return (
    <View style={styles.payBox}>
      ...
      <Text style={[type.display, { color: colors.onPaper }]}>₹{amount}</Text>
      {noApp && <Notice tone="warn" title={t('noUpiApp')} />}
      {upiOpened ? (
        <>
          <Text style={[type.title, { color: colors.onPaper }]}>{t('upiDidItWork')}</Text>
          <PrimaryButton label={t('upiYes')} tone="green" onPress={() => paid('upi')} />
        </>
      ) : (
        upi && !noApp && <PrimaryButton label={t('payUpi', { amount })} onPress={openUpi} />
      )}
      <PrimaryButton label={t('paidCash', { amount })} tone="ink" onPress={() => paid('cash')} />
    </View>
  );
}
...
function Rate({ job, onRated }: { job: Job; onRated: () => void }) {
  const { lang } = usePrefs();
  const { t } = useTranslation();
  const type = typeScale(lang);
  ...
  const send = () => {
    setBusy(true);
    rateJob(job, stars, picked).then(onRated).catch(() => setBusy(false));
  };

  return (
    <View style={styles.payBox}>
      <Text style={[type.title, { color: colors.onPaper }]}>{t('rateTitle', { name: job.worker?.name.split(' ')[0] ?? '' })}</Text>
      ...
      <PrimaryButton label={t('sendRating')} onPress={send} disabled={stars === 0 || busy} />
    </View>
  );
}
